...
// const ssl = @import("bear.zig");

const c = @cImport({
    @cInclude("bearssl.h");
});

const BearError = error{
    BAD_PARAM,
    BAD_STATE,
    UNSUPPORTED_VERSION,
    BAD_VERSION,
...
pub fn main() !u8 {
    ...
    var trust_anchors = std.ArrayList(c.br_x509_trust_anchor).init(generic_allocator);
...

    {
        const pem_text = blk: {
            ...
            break :blk try file.inStream().readAllAlloc(generic_allocator, 1 << 20); // 1 MB
        };
        defer generic_allocator.free(pem_text);

        var objectBuffer = std.ArrayList(u8).init(generic_allocator);
                ...
                c.BR_PEM_END_OBJ => {
                    if (current_obj_is_certificate) {
                        var certificate = c.br_x509_certificate{
                            .data = (try std.mem.dupe(generic_allocator, u8, objectBuffer.items)).ptr,
                            .data_len = objectBuffer.items.len,
                        };
                        defer generic_allocator.free(certificate.data[0..certificate.data_len]);

                        var trust_anchor = try convertToTrustAnchor(generic_allocator, certificate);
    ...
    var client: c.br_ssl_client_context = undefined;
    var x509: c.br_x509_minimal_context = undefined;
    c.br_ssl_client_init_full(&client, &x509, trust_anchors.items.ptr, trust_anchors.items.len);

    var iobuf: [c.BR_SSL_BUFSIZE_BIDI]u8 = undefined;
    c.br_ssl_engine_set_buffer(&client.eng, &iobuf, iobuf.len, 1);

    _ = c.br_ssl_client_reset(&client, "gemini.circumlunar.space", 0);
    ...
    var ssl_stream = SSLStream.init(&client.eng, socket.internal);
